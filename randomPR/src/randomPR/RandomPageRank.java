...
public class RandomPageRank {    
    ...
    public static void main(String[] args) throws Exception {
    	RandomPageRank rank = new RandomPageRank();
    ...
    public ArrayList<Double> runPageRankCalculation(String InputPath, String OutputPath) throws Exception {   
        ...
        conf.setJarByClass(RandomPageRank.class);
        ...
        conf.setMapperClass(RandomPageRankMap.class);
        conf.setReducerClass(RandomPageRankReduce.class);
        ...
        Counter c = counters.findCounter(PRCounter.GLOBAL_RESIDUAL_COUNTER);
        double totalresidual = c.getValue() / (1.0*10e6);
        
        c = counters.findCounter(PRCounter.LOCAL_ITERATION_NUM_COUNTER);
        long iter_sum = c.getValue();
        c = counters.findCounter(PRCounter.BLOCK_COUNTER);
        ...
        for (int i = 0; i < block_num; ++ i) {
        	c = counters.findCounter(BlockCounter.values()[i]);
